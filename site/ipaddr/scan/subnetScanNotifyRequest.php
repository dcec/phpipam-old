...
if ($mysqli->connect_errno) {
	...
	$requests = getAllReservedIPrequests("30");
foreach($requests as $request) {
	...
	/* set title */
	$title = _('IP reserved inactive').' : ' . $ip['ip_addr'];
	...
	$content .= "\nThis ip will be deleted if won't become active within ".$request['endDate']."!";
	
	foreach ($recipients_temp as $rec) {
	//verify each email
		if(!checkEmail($rec)) {$errors[] = $rec;}
	}
	if(!$_POST){$user = "Crontab";}else{$user = NULL;}
	
	if(!$_POST){
	if (!$errors) {
	if(!sendIPnotifEmail($request['requester'], $title, $content))	{ 
		print '<div class="alert alert-danger">'._('Sending mail failed').'!</div>'; 
		updateLogTable ('Error on notification for inactive reserved IP address '. $ip['ip_addr'], 'Error on notification for inactive reserved IP address '. $ip['ip_addr'] .'<br>SubnetId: '. $ip['subnetId'], 2,$user);
		}
	else{ 
		print '<div class="alert alert-success">'._('Sending mail succeeded').'!</div>';
		updateLogTable ('Notification for inactive reserved IP address '. $ip['ip_addr'] .' succesfull!', 'Notification for inactive reserved IP address '. $ip['ip_addr'] .' succesfull!<br>SubnetId: '. $ip['subnetId'], 0,$user);
	}
	}else{
